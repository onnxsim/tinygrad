 ...
 *
 * This is the platform half of the HMX pair: hmx_block.h has the tile ops and says nothing about how
 * VTCM or the power votes are obtained, because a kernel that did would be untestable off-device. It
 * lives here, next to the kernels, because the two are one contract - hmx_block.h points at this file
 * for its preconditions, and every skel that runs a hand or generated HMX kernel includes both.
 *
 * Not compiled by the oracle tests: HAP_power / HAP_compute_res / qurt are FastRPC and QuRT headers,
 * present only in the phone SDK, so this is exercised by the phone runs (onnxsim's
 * scripts/android/hmx_gemm and tinygrad_hexagon_bridge/tinygrad_codegen/hmx) rather than by
 * hexagon-sim or qemu.
 *
 ...
 * rebooted (onnxsim scripts/android/hmx_probe, PRs #1890 / #1905). */
